...
class OptimalExplorer {
    ...
    private int c;              // starting column
    private int[][] cost;       // map altitudes

    private int[][] effort;     // effort[i, j] = minimum effort required to reach square (i, j)
    private int[][] prev;       // prev[i, j]   = previous square (left, up or right)
    ...
    OptimalExplorer(String inputPath) {
        readInput(inputPath);
    }
    ...
    void traceback() {
        ...
        List<Point> path = new ArrayList<>(m);
        int col = minIndex;
    ...
    private void printConfig(int row, int col) {
        System.out.println("\n     Cost:          Effort:          Prev:");
    ...
    private void printHorizontalSeparator(int count) {
        System.out.print(" ");
